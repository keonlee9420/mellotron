"""
This is from https://github.com/NVIDIA/tacotron2/issues/263#issuecomment-529749345.
...
def preprocess_audio(file_list, silence_audio_size, pre_emphasis=False):
    for F in file_list:
        ...
        for i, r in enumerate(R):
            ...
            data_ = librosa.effects.trim(data, top_db=trim_top_db,
                                         frame_length=trim_fft_size, hop_length=trim_hop_size)[0]
            if pre_emphasis:
...
if __name__ == "__main__":
    """
    python preprocess.py -f=filelist.txt -s=5 -t -p -r
    """
    ...
    parser.add_argument('-s', '--silence_padding', type=int, default=0,
                        help='Adding silence padding at the end of each audio, '
                             'silence audio size is hop_length * silence padding')
